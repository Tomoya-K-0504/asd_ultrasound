...
def preprocess(expt_conf):
    ...
    output_dir = Path(__file__).resolve().parents[1] / 'input' / 'split'
    ...
    for spect_path in tqdm(spect_list):
        ...
        for i_frame in range(n_frames):
            ...
            np.save(output_dir / file_name, frame)
            manifest_list.append((output_dir / file_name, label))

    pd.DataFrame(manifest_list, columns=['file_path', 'label']).to_csv(input_dir / 'manifest.csv', index=False)
